...
<label><input type="checkbox" id="update-gl-and-update-modified" <% if (updateModified) { %>checked<% } %>> 確認後にmodifiedタグが付いた小説をUpdate</label>
  ...
  $.extend(Console.prototype, {
    ...
    init_notification: function() {
      ...
      // AJAX polling fallback - works without WebSocket (port+1) forwarding
      this._poll_timer = setInterval(function() {
        if (self.notification.connected) return;
        $.get("/api/history", { stream: self.options.stream_id }, function(history) {
          if (!history) return;
          var lines = history.trim().split("\n");
          if (lines.length > self._history_len) {
            var new_lines = lines.slice(self._history_len);
            self.puts(new_lines.join("\n") + "\n");
            self._history_len = lines.length;
          }
        });
      }, 2000);
    },
